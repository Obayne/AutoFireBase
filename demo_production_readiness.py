...
def create_realistic_building_analysis():
    ...
    class Room:
        def __init__(self, name, room_type, area, classification):
            ...
            self.occupancy_type = self.map_to_occupancy_type(classification)  # Required by AI processor
        ...
        def determine_requirements(self):
            ...
            if self.classification == "critical":
                ...
                requirements.extend(["ada_compliance", "emergency_communication", "security_monitoring"])
            elif self.room_type == "open_office":
                requirements.extend(["distributed_power", "data_connectivity", "wireless_infrastructure"])
    ...
    try:
        ...
        if simplified_plans:
            ...
            for plan in simplified_plans:
                ...
                print(f"• {sheet_num}: {len(zones)} low voltage zones, {total_zone_area:,.0f} sq ft")
                ...
                if zone_types:
                    print(f"  Zone types: {', '.join([f'{t}: {c}' for t, c in zone_types.items()])}")
        ...
        if device_types:
            ...
            if len(device_types) > 10:
                ...
                print(f"• ... and {len(device_types) - 10} more device types: ${remaining_cost:,.2f}")
