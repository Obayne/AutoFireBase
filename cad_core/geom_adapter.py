...
"""Geometry adapter: prefer lv_cad implementation when available.

This file exposes small helpers built on Shapely. During the strangler
migration we prefer the `lv_cad` implementations when present and fall
back to the legacy code below for environments where `lv_cad` hasn't
been installed or imported yet.
"""
...
_lv = None
try:
    _lv = importlib.import_module("lv_cad.cad_core.geom_adapter")
except Exception:
    _lv = None

if _lv is not None:
    segment_intersection = _lv.segment_intersection
    trim_segment_to_intersection = _lv.trim_segment_to_intersection
