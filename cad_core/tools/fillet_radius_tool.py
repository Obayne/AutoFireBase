...
class FilletRadiusTool:
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        ip = QtCore.QPointF()
        if (
            l1.intersect(l2, ip) != QtCore.QLineF.IntersectType.BoundedIntersection
            and l1.intersect(l2, ip) != QtCore.QLineF.IntersectType.UnboundedIntersection
        ):
            self.active = False
            self.first = None
            return False
        ...
        def trim_point(line: QtCore.QLineF):
            ...
            da = QtCore.QLineF(ip, a).length()
            db = QtCore.QLineF(ip, b).length()
        ...
        update_line(self.first, l1, p1)
        update_line(it, l2, p2)
        ...
        arc_item.setPen(pen)
        arc_item.setZValue(20)
        arc_item.setParentItem(self.layer)
