...
class WindowManager:
    ...
    def _position_main_window(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_properties_panel(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_device_tree(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
        """Position device tree on left side."""
        x = screen_geom.x() + 20
        y = screen_geom.y() + 100
        return QPoint(x, y), screen
    
    def _position_calculations(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_reports(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_dialog(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_tool_palette(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def _position_default(self, size: QSize, screen_geom: QRect, screen: QScreen) -> Tuple[QPoint, QScreen]:
    ...
    def create_dock_widget(self, title: str, widget: QWidget, 
                          dock_id: str, position: DockPosition = DockPosition.RIGHT) -> QDockWidget:
    ...
    def _apply_dual_monitor_layout(self):
        ...
        if len(self.screens) < 2:
            QMessageBox.information(None, "Monitor Setup", 
...
class WindowManagementDemo(QMainWindow):
    ...
    def setup_ui(self):
        ...
        info.setStyleSheet(f"color: {AutoFireColor.TEXT_SECONDARY}; padding: 10px; line-height: 1.4;")
        ...
        create_properties.clicked.connect(lambda: self.create_demo_window("properties", "Properties"))
        button_layout.addWidget(create_properties)
        
        create_calculations = QPushButton("🧮 Open Calculations")
        create_calculations.clicked.connect(lambda: self.create_demo_window("calculations", "Live Calculations"))
        button_layout.addWidget(create_calculations)
        
        create_reports = QPushButton("📊 Open Reports")
        create_reports.clicked.connect(lambda: self.create_demo_window("reports", "Professional Reports"))
        ...
        designer_btn.clicked.connect(lambda: self.window_manager.apply_layout_profile(WindowProfile.DESIGNER))
        profile_layout.addWidget(designer_btn)
        
        engineer_btn = QPushButton("🧮 Engineer Layout")
        engineer_btn.clicked.connect(lambda: self.window_manager.apply_layout_profile(WindowProfile.ENGINEER))
        profile_layout.addWidget(engineer_btn)
        
        manager_btn = QPushButton("📊 Manager Layout")
        manager_btn.clicked.connect(lambda: self.window_manager.apply_layout_profile(WindowProfile.MANAGER))
        profile_layout.addWidget(manager_btn)
        
        dual_monitor_btn = QPushButton("🖥️ Dual Monitor")
        dual_monitor_btn.clicked.connect(lambda: self.window_manager.apply_layout_profile(WindowProfile.DUAL_MONITOR))
        ...
        status_text.setStyleSheet(f"color: {AutoFireColor.TEXT_SECONDARY}; padding: 10px; background: {AutoFireColor.SURFACE}; border-radius: 5px;")
    ...
    def setup_menu(self):
        ...
        profiles_menu.addAction("🎨 Designer", lambda: self.window_manager.apply_layout_profile(WindowProfile.DESIGNER))
        profiles_menu.addAction("🧮 Engineer", lambda: self.window_manager.apply_layout_profile(WindowProfile.ENGINEER))
        profiles_menu.addAction("📊 Manager", lambda: self.window_manager.apply_layout_profile(WindowProfile.MANAGER))
        profiles_menu.addAction("🖥️ Dual Monitor", lambda: self.window_manager.apply_layout_profile(WindowProfile.DUAL_MONITOR))
    ...
    def create_demo_window(self, window_type: str, title: str):
        ...
        header.setStyleSheet(f"font-size: 14px; font-weight: bold; color: {AutoFireColor.PRIMARY}; padding: 10px;")
        layout.addWidget(header)
        
        content = QLabel(f"This is a demonstration {window_type} window.\n\n"
