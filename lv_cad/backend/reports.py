"""Reporting helpers migrated to lv_cad.backend (parity copy)."""
...
def generate_bom_csv(devices: Iterable[Any], out_path: str) -> dict[str, Any]:
    ...
    return {"bom_path": out_path, "unique_items": len(counts), "total_qty": total_qty}
    ...
    with zipfile.ZipFile(zip_path, "w") as z:
        for p in [
            ...
            if p and os.path.exists(p):
                ...
                name = os.path.basename(p) if p else "placeholder.txt"
                z.writestr(name, "Missing file placeholder")
        html = export_html_submittal(device_items, wire_items, tmp_dir)
        index_path = html.get("index_path")
...
def generate_riser_csv(*_args: Any, out_path: str) -> dict[str, Any]:
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
...
def generate_compliance_summary_csv(*_args: Any, out_path: str) -> dict[str, Any]:
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("Compliance placeholder\n")
