...
class AutoFireTheme:
    ...
    def to_dict(self) -> Dict:
        ...
        return {"metadata": metadata_dict, "colors": asdict(self.colors)}
...
class ColorPicker(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.color_display.setStyleSheet(
            f"background-color: {self.current_color}; border: 2px solid #ccc;"
        )
...
class ThemeEditor(QWidget):
        ...
        for group_name, color_names in color_groups.items():
            ...
            for i, color_name in enumerate(color_names):
                ...
                picker.color_changed.connect(
                    lambda color, name=color_name: self._on_color_changed(name, color)
                )
    ...
    def _setup_advanced_tab(self):
        ...
        self.ui_scale_slider.valueChanged.connect(
            lambda v: self.ui_scale_label.setText(f"{v/100:.1f}x")
        )
    ...
    def _export_theme(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Export Complete", f"Theme exported to {file_path}")
            except Exception as e:
                QMessageBox.warning(self, "Export Failed", f"Error: {str(e)}")

    def _reset_theme(self):
        """Reset theme to default."""
        reply = QMessageBox.question(self, "Reset Theme", "Reset all colors to default values?")
...
class ThemeManager(QWidget):
    ...
    def _filter_themes(self, category_text: str):
        ...
        for theme in self.available_themes:
            if (
                category_text == "All Themes"
                or theme.metadata.category.value.replace("_", " ").title() == category_text
            ):
                item = QListWidgetItem(f"🎨 {theme.metadata.display_name}")
                item.setData(Qt.UserRole, theme)
                self.theme_list.addItem(item)
    ...
    def _display_theme_details(self, theme: AutoFireTheme):
        ...
        self.theme_category.setText(
            f"Category: {theme.metadata.category.value.replace('_', ' ').title()}"
        )
    ...
    def _import_theme(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Import Theme", "", "AutoFire Theme Files (*.autofire_theme);;JSON Files (*.json)"
        )
