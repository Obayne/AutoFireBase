...
class ProfessionalReportingSystem(QMainWindow):
    ...
    def load_default_templates(self) -> List[ReportTemplate]:
        ...
        templates = [
            ...
            ReportTemplate(
                ...
                columns=["Address", "Device Type", "Model", "Location", "Circuit", "Zone", "Voltage", "Current", "Notes"],
            ...
            ReportTemplate(
                ...
                columns=["Circuit ID", "Type", "Wire Gauge", "Length (ft)", "Devices", "Load (A)", "V-Drop (%)", "Status"],
            ...
            ReportTemplate(
                ...
                columns=["Device", "Location", "Height", "Mounting", "Wire Routing", "Special Instructions"],
    ...
    def load_sample_data(self) -> Dict[str, List]:
        ...
        devices = [
            DeviceRecord("01-001", "Smoke Detector", "SIGA-PS", "Conference Room A", "SLC-1", "Zone 1", "24V", "220µA", "Ceiling mount"),
            DeviceRecord("01-002", "Smoke Detector", "SIGA-PS", "Conference Room B", "SLC-1", "Zone 1", "24V", "220µA", "Ceiling mount"),
            DeviceRecord("01-003", "Horn/Strobe", "MASS-24-15/75", "Corridor - North", "NAC-1", "Zone 1", "24V", "0.095A", "Wall mount 80\" AFF"),
            DeviceRecord("01-004", "Horn/Strobe", "MASS-24-15/75", "Corridor - South", "NAC-1", "Zone 1", "24V", "0.095A", "Wall mount 80\" AFF"),
            DeviceRecord("01-005", "Pull Station", "M5A-R", "Main Exit", "SLC-1", "Zone 1", "24V", "50µA", "48\" AFF per ADA"),
            DeviceRecord("01-006", "Monitor Module", "SIGA-MIM", "Sprinkler Flow Switch", "SLC-1", "Zone 2", "24V", "50µA", "Electrical room"),
            DeviceRecord("01-007", "Control Module", "SIGA-CRM", "Fire Pump Start", "SLC-1", "Zone 2", "24V", "50µA", "Fire pump room"),
            DeviceRecord("02-001", "Smoke Detector", "SIGA-PS", "Office Area - East", "SLC-2", "Zone 2", "24V", "220µA", "Ceiling mount"),
            DeviceRecord("02-002", "Smoke Detector", "SIGA-PS", "Office Area - West", "SLC-2", "Zone 2", "24V", "220µA", "Ceiling mount"),
            DeviceRecord("02-003", "Speaker/Strobe", "MASS-24VSR-15/75", "Reception Area", "NAC-2", "Zone 2", "24V", "0.125A", "Wall mount 80\" AFF")
        ]
    ...
    def create_preview_panel(self) -> QWidget:
        ...
        self.template_info.setStyleSheet(f"""
            QLabel {{
    ...
    def create_report_header(self) -> QWidget:
        ...
        project_details = QLabel(f"Project: {project_info['name']}\\nLocation: {project_info['address']}\\nEngineer: {project_info['engineer']}\\nDate: {project_info['date']}")
        project_details.setStyleSheet(f"""
            QLabel {{
                color: {AutoFireColor.TEXT_SECONDARY.value};
                font-size: 11px;
                line-height: 1.4;
    ...
    def get_available_columns(self, report_type: ReportType) -> List[str]:
        ...
        if report_type == ReportType.DEVICE_SCHEDULE:
            return ["Address", "Device Type", "Model", "Location", "Circuit", "Zone", "Voltage", "Current", "Notes"]
        elif report_type == ReportType.CIRCUIT_ANALYSIS:
            return ["Circuit ID", "Type", "Wire Gauge", "Length (ft)", "Devices", "Load (A)", "V-Drop (%)", "Status"]
        else:
    ...
    def populate_device_schedule(self, template: ReportTemplate):
        ...
        self.report_footer.setText(f"Summary: {device_count} devices across {circuit_count} circuits. All devices meet NFPA 72 requirements for addressable fire alarm systems.")
    ...
    def populate_circuit_analysis(self, template: ReportTemplate):
        ...
        self.report_footer.setText(f"Circuit Analysis Summary: {len(circuits)} circuits serving {total_devices} devices. Maximum voltage drop: {max_voltage_drop:.1f}% (NFPA 72 limit: 10%).")
    ...
    def populate_generic_report(self, template: ReportTemplate):
        ...
        self.report_table.resizeColumnsToContents()
        self.report_footer.setText("Sample report data. Select a specific report type for actual project data.")
    ...
    def save_current_template(self):
        ...
        self.statusBar().showMessage(f"Template '{self.current_template.name}' saved successfully", 3000)
    ...
    def copy_to_clipboard(self):
        ...
        self.statusBar().showMessage("Report table copied to clipboard - ready to paste into Excel or CAD", 5000)
        
    def cad_export(self, export_type: str):
        """Handle CAD export options.""" 
        if "CAD Block" in export_type:
            self.statusBar().showMessage("Generating AutoCAD block for model space insertion...", 3000)
