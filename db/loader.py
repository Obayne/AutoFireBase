...
def seed_demo(con: sqlite3.Connection):
    ...
    def add(dev):
        ...
        cur.execute(
            ...
            (mfr_id, t_id, c_id, 1, NULL, dev.get('part_number',''), dev['name'], dev['symbol'], json.dumps(dev.get('props',{})), dev.get('panel_standby_current_ma',0.0), dev.get('panel_alarm_current_ma',0.0))
        )
    ...
    cur.execute("INSERT OR IGNORE INTO layers (name, color, visible, locked, show_name, show_part_number) VALUES (?, ?, ?, ?, ?, ?)", ("0", "#FFFFFF", True, False, True, True))
    
    # Seed wire specs (gauge to resistance per 1000ft)
    cur.executemany("INSERT OR IGNORE INTO wire_specs(gauge, resistance_per_1000ft) VALUES(?,?)", [
...
def seed_wires(con: sqlite3.Connection):
    ...
    cur.executemany("INSERT OR IGNORE INTO wires(part_number, manufacturer, type, gauge, color) VALUES(?,?,?,?,?)", wires)
    con.commit()
...
def register_block_for_device(con: sqlite3.Connection, device_id: int, block_name: str, block_path: str, attributes: dict | None = None) -> int:
...
def get_block_for_device(con: sqlite3.Connection, device_id: int) -> dict | None:
    ...
    if row:
        result = {
            ...
            'block_attributes': json.loads(row['block_attributes']) if row['block_attributes'] else {}
        }
        return result
    return None
...
def save_job_info(con: sqlite3.Connection, project_name: str, project_address: str, sheet_number: str, drawing_date: str, drawn_by: str):
    cur = con.cursor()
    cur.execute("INSERT OR REPLACE INTO job_info (id, project_name, project_address, sheet_number, drawing_date, drawn_by) VALUES (1, ?, ?, ?, ?, ?)",
...
def save_circuit(con: sqlite3.Connection, panel_id: int, circuit_type: str, capacity: int, cable_length: float):
    cur = con.cursor()
    cur.execute("INSERT OR REPLACE INTO circuits (panel_id, circuit_type, capacity, cable_length) VALUES (?, ?, ?, ?)",
