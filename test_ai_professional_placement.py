...
from cad_core.spaces import ModelSpace, PaperSpace, PageSize
from cad_core.spaces.model_space import Line, Bounds
from cad_core.ai.device_placement import DeviceType, SpaceType
from cad_core.ai.professional_placement import (
    ProfessionalAIPlacementEngine, FireAlarmEntity, create_professional_ai_engine
)
...
def test_smoke_detector_placement():
    ...
    print(f"✓ AI placed {result.total_devices} smoke detectors with {result.coverage_percentage:.1f}% coverage")
...
def test_multi_device_type_placement():
    ...
    total_devices = (smoke_result.total_devices + 
                    pull_result.total_devices + 
                    notification_result.total_devices)
    ...
    print(f"✓ AI placed {smoke_result.total_devices} detectors, "
...
def test_ai_paper_space_integration():
    ...
    for entity in model.entities.values():
        if isinstance(entity, FireAlarmEntity):
            ...
            assert viewport.is_point_in_viewport(paper_pos), \
                f"Device at {model_pos} not visible in viewport"
    
    total_devices = (smoke_result.total_devices + 
                    pull_result.total_devices + 
                    horn_result.total_devices)
...
def test_nfpa_compliance_validation():
    ...
    assert result_proper.total_devices >= min_devices, \
    ...
    print(f"✓ NFPA compliance validated - {result_proper.total_devices} devices, "
...
def test_performance_and_statistics():
    ...
    assert abs(actual_smoke - expected_smoke_devices) < expected_smoke_devices * 0.5, \
        "Device count significantly different from NFPA estimates"
    
    print(f"✓ Performance validated - {total_time:.2f}s for {stats['total_devices_placed']} devices")
