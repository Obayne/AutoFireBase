# apply_cad_step1b_065.py
# Improves:
# - Real hand-pan (space+left drag or middle drag)
# - Device selection visibility (cyan ring + hover glow, slightly larger hit)
# - Theme selector (Settings -> Theme), persists to preferences
# Also keeps CAD step 1 shortcuts from previous patch.
#
# Overwrites (with backups): app/main.py, app/device.py, app/tools/transform.py
...
class DeviceItem(QtWidgets.QGraphicsItemGroup):
    def __init__(self, x: float, y: float, symbol: str, name: str, manufacturer: str = "", part_number: str = ""):
        ...
        # Base glyph (slightly larger to make selection easier)
        self._glyph = QtWidgets.QGraphicsEllipseItem(-7, -7, 14, 14)
        ...
        # Hover ring (subtle)
        self._hover_ring = QtWidgets.QGraphicsEllipseItem(-11, -11, 22, 22)
        hpen = QtGui.QPen(QtGui.QColor(120, 200, 255, 140))
        hpen.setCosmetic(True)
        self._hover_ring.setPen(hpen)
        self._hover_ring.setBrush(QtCore.Qt.NoBrush)
        self._hover_ring.setVisible(False)
        self._hover_ring.setZValue(990)
        self.addToGroup(self._hover_ring)

        # Selection ring (strong cyan)
        self._sel_ring = QtWidgets.QGraphicsEllipseItem(-12, -12, 24, 24)
        spen = QtGui.QPen(QtGui.QColor(50, 210, 255, 230))
        spen.setCosmetic(True)
        spen.setWidthF(1.5)
        self._sel_ring.setPen(spen)
        self._sel_ring.setBrush(QtCore.Qt.NoBrush)
        self._sel_ring.setVisible(False)
        self._sel_ring.setZValue(995)
    ...
    def _ensure_cov_items(self):
        if self._cov_circle is None:
            ...
            self._cov_circle.setBrush(QtGui.QColor(50,120,255,60))
        if self._cov_square is None:
            ...
            self._cov_square.setBrush(QtGui.QColor(50,120,255,30))
        if self._cov_rect is None:
            ...
            self._cov_rect.setBrush(QtGui.QColor(50,120,255,30))
    ...
    def itemChange(self, change, value):
        if change == QtWidgets.QGraphicsItem.ItemSelectedHasChanged:
            sel = bool(self.isSelected())
            self._sel_ring.setVisible(sel)
...
def _clone_item(it: QtWidgets.QGraphicsItem) -> QtWidgets.QGraphicsItem | None:
    if isinstance(it, DeviceItem):
        ...
        if "rotation" in d:
            try: clone.setRotation(float(d["rotation"]))
            except Exception: pass
        return clone
...
class CanvasView(QGraphicsView):
    def __init__(self, scene, devices_group, wires_group, sketch_group, overlay_group, window_ref):
        ...
        # custom pan state
        self._hand_pan_ready = False  # space held down?
        self._pan_active = False      # actually dragging
        self._pan_last = None
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # apply theme AFTER view created
...
def write_file(rel, content):
    ...
    if path.exists():
        ...
        try:
            ...
            print(f"warn: could not backup {path}: {ex}")
