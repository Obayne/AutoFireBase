# apply_hotfix_fallback_066.py
# Purpose: stop fallback window by guarding optional imports used in app.main.
...
try:
    ...
    class ArrayTool:
        def __init__(self, *a, **k):
            self.pending = False
        def run(self): self.pending = False
        def cancel(self): self.pending = False
        def on_click(self, *a, **k): return False
...
try:
    ...
    class CoverageDialog(QtWidgets.QDialog):
        def __init__(self, parent=None, existing=None, px_per_ft=12.0):
            ...
            self._settings = {"mode":"none","computed_radius_px":0.0}
            txt = QtWidgets.QLabel(
                "Coverage dialog placeholder. (Real dialog will be added.)"
            )
            ...
            lay = QtWidgets.QVBoxLayout(self); lay.addWidget(txt); lay.addWidget(b)
        def get_settings(self): return self._settings

from app import units

APP_VERSION = "0.6.6-fallback-hotfix"
...
THEMES = {
    "dark": {
        "window": (32,32,36), "base": (26,26,28), "text": (255,255,255),
        "button": (48,48,52), "button_text": (255,255,255),
        "bg_brush": (34,34,38)
    },
    "medium": {
        "window": (38,38,44), "base": (32,32,36), "text": (240,240,240),
        "button": (56,56,62), "button_text": (240,240,240),
        "bg_brush": (40,40,46)
    },
    "slate": {
        "window": (58,62,68), "base": (52,56,62), "text": (15,15,18),
        "button": (210,214,220), "button_text": (15,15,18),
        "bg_brush": (210,214,220)
    }
}

def apply_theme_to(self, theme_key: str):
    ...
    pal.setColor(QtGui.QPalette.Button, QtGui.QColor(*theme["button"]))
    pal.setColor(QtGui.QPalette.ButtonText, QtGui.QColor(*theme["button_text"]))
    self.setPalette(pal)
    if getattr(self, "view", None):
        self.view.setBackgroundBrush(QtGui.QColor(*theme["bg_brush"]))
...
class CanvasView(QGraphicsView):
    def __init__(self, scene, devices_group, wires_group, sketch_group, overlay_group, window_ref):
        ...
        # custom pan state
        self._hand_pan_ready = False  # space held?
        self._pan_active = False      # dragging?
        self._pan_last = None
    ...
    def _end_pan(self):
        ...
        self._pan_last = None
        self.viewport().setCursor(Qt.OpenHandCursor if self._hand_pan_ready else Qt.ArrowCursor)

    def mousePressEvent(self, e: QtGui.QMouseEvent):
        pos = e.position().toPoint()
        if e.button() == Qt.MiddleButton:
            self._begin_pan(pos); e.accept(); return
        if e.button() == Qt.LeftButton and self._hand_pan_ready:
            self._begin_pan(pos); e.accept(); return

        win = self.win
        sp = self.scene().snap(self.mapToScene(pos))
        if e.button()==Qt.LeftButton:
            if getattr(win, "array_tool", None) and getattr(win.array_tool, "pending", False):
    ...
    def mouseReleaseEvent(self, e: QtGui.QMouseEvent):
        if self._pan_active and (e.button() in (Qt.LeftButton, Qt.MiddleButton)):
            self._end_pan(); e.accept(); return
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent):
        ...
        if e.key()==Qt.Key_Space and not self._hand_pan_ready:
            self._hand_pan_ready = True
            self.viewport().setCursor(Qt.OpenHandCursor); e.accept(); return
        if e.key()==Qt.Key_Escape:
            if getattr(self.win, "draw", None) and self.win.draw.mode != draw_tools.DrawMode.NONE:
                self.win.draw.finish(); e.accept(); return
            if getattr(self.win, "array_tool", None) and getattr(self.win.array_tool, "pending", False):
    ...
    def keyReleaseEvent(self, e: QtGui.QKeyEvent):
        ...
        if e.key()==Qt.Key_Space and self._hand_pan_ready and not self._pan_active:
            self._hand_pan_ready = False
            self.viewport().setCursor(Qt.ArrowCursor); e.accept(); return
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        apply_theme_to(self, self.theme)
        ...
        for key, text in [("dark","Dark (default)"), ("medium","Medium Dark"), ("slate","Slate Light")]:
            a = _theme_action(key, text); group.addAction(a); m_theme.addAction(a)
        ...
        QtGui.QShortcut(QtGui.QKeySequence(Qt.Key_Up), self, activated=lambda: self.nudge(0, -1, False))
        QtGui.QShortcut(QtGui.QKeySequence(Qt.Key_Down), self, activated=lambda: self.nudge(0, +1, False))
        QtGui.QShortcut(QtGui.QKeySequence(Qt.Key_Left), self, activated=lambda: self.nudge(-1, 0, False))
        QtGui.QShortcut(QtGui.QKeySequence(Qt.Key_Right), self, activated=lambda: self.nudge(+1, 0, False))
        QtGui.QShortcut(QtGui.QKeySequence("Shift+Up"), self, activated=lambda: self.nudge(0, -1, True))
        QtGui.QShortcut(QtGui.QKeySequence("Shift+Down"), self, activated=lambda: self.nudge(0, +1, True))
        QtGui.QShortcut(QtGui.QKeySequence("Shift+Left"), self, activated=lambda: self.nudge(-1, 0, True))
        QtGui.QShortcut(QtGui.QKeySequence("Shift+Right"), self, activated=lambda: self.nudge(+1, 0, True))
    ...
    def dup_selected(self):
        dx_px = units.ft_to_px(0.5, self.px_per_ft)
