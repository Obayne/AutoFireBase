...
class ScaleTool:
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        if self.base is None:
            self.base = p
            val, ok = QtWidgets.QInputDialog.getDouble(self.win, "Scale", "Factor", 1.0, 0.01, 1000.0, 3)
            if not ok:
            ...
            t = QtGui.QTransform()
            t.translate(cx, cy); t.scale(f, f); t.translate(-cx, -cy)
            for it in sel:
                try: it.setTransform(t, combine=True)
