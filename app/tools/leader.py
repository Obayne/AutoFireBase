...
class LeaderTool:
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        line.setPen(pen); line.setZValue(20); line.setParentItem(self.layer)
        line.setFlag(QtWidgets.QGraphicsItem.ItemIsSelectable, True)
        line.setFlag(QtWidgets.QGraphicsItem.ItemIsMovable, True)
        # arrow head
        v = QtCore.QLineF(self.p1, self.p0); v.setLength(12)
        left = v.normalVector(); left.setLength(6); right = v.normalVector(); right.setLength(-6)
        p2 = v.p2(); lpt = QtCore.QPointF(p2.x()+left.dx(), p2.y()+left.dy())
        ...
        head.setBrush(QtGui.QColor("#e0e0e0")); head.setPen(QtGui.QPen(QtCore.Qt.NoPen))
        head.setZValue(20); head.setParentItem(self.layer)
        # text
