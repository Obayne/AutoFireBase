...
def align_right(items):
    _align(items, key=lambda it: it.rect().right(), setter=lambda it, x: _move_x(it, x - it.rect().width()))

def align_top(items):
    _align(items, key=lambda it: it.rect().top(), setter=lambda it, y: _move_y(it, y))

def align_bottom(items):
    _align(items, key=lambda it: it.rect().bottom(), setter=lambda it, y: _move_y(it, y - it.rect().height()))

def distribute_h(items):
    if len(items) < 3: return
    items = sorted(items, key=lambda it: it.rect().left())
    left = items[0].rect().left(); right = items[-1].rect().left()
...
def distribute_v(items):
    ...
    top = items[0].rect().top(); bottom = items[-1].rect().top()
...
def _move_x(it, x):
    r = it.rect(); it.setPos(x, r.top())

def _move_y(it, y):
    r = it.rect(); it.setPos(r.left(), y)
