...
class ChamferTool:
    ...
    def start(self):
        self.active = True
        self.first = None
        dlg = QtWidgets.QDialog(self.win); dlg.setWindowTitle("Chamfer")
        form = QtWidgets.QFormLayout(dlg)
        s1 = QtWidgets.QDoubleSpinBox(); s1.setRange(0, 1000); s1.setDecimals(2); s1.setValue(1.0)
        s2 = QtWidgets.QDoubleSpinBox(); s2.setRange(0, 1000); s2.setDecimals(2); s2.setValue(1.0)
        form.addRow("Distance 1 (ft):", s1); form.addRow("Distance 2 (ft):", s2)
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        l1 = QtCore.QLineF(self.first.line()); l2 = QtCore.QLineF(it.line())
        ip = QtCore.QPointF()
        ...
        def move_line(line: QtCore.QLineF, dist_ft: float, px_per_ft: float):
            ...
            da = QtCore.QLineF(a, ip).length(); db = QtCore.QLineF(b, ip).length()
            dpx = dist_ft * px_per_ft
            if da < db:
                v = QtCore.QLineF(ip, a); ln = v.length() or 1.0; ux = (a.x()-ip.x())/ln; uy = (a.y()-ip.y())/ln
                a = QtCore.QPointF(ip.x()+ux*dpx, ip.y()+uy*dpx)
            else:
                v = QtCore.QLineF(ip, b); ln = v.length() or 1.0; ux = (b.x()-ip.x())/ln; uy = (b.y()-ip.y())/ln
