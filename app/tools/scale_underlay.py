...
class ScaleUnderlayRefTool:
    ...
    def start(self):
        ...
        self.win.statusBar().showMessage("Underlay Scale (Ref): click first point, then second point")
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        if self.p2 is None:
            ...
            val, ok = QtWidgets.QInputDialog.getDouble(self.win, "Real Distance", "Distance (feet)", 10.0, 0.01, 100000.0, 3)
            if not ok:
                self.active=False; return False
...
def scale_underlay_by_factor(group: QtWidgets.QGraphicsItemGroup, factor: float, anchor: QtCore.QPointF = QtCore.QPointF(0,0)):
    t = QtGui.QTransform()
...
class ScaleUnderlayDragTool:
    ...
    def start(self):
        ...
        self.win.statusBar().showMessage("Underlay Scale (Drag): click anchor point, then move mouse; click again to commit, Esc to cancel")

    def cancel(self):
        if self.active and self.orig is not None:
            try: self.group.setTransform(self.orig)
    ...
    def on_click(self, p: QtCore.QPointF):
        ...
        self.active = False; self.anchor=None; self.orig=None
        return True
