    ...
    def on_mouse_move(self, pt_scene: QtCore.QPointF, shift_ortho=False):
        ...
        if self.mode in (DrawMode.LINE, DrawMode.WIRE):
            ...
            if self.temp_item is None:
                ...
                pen = QtGui.QPen(QtGui.QColor("#bb9af7"))
                pen.setCosmetic(True)
                self.temp_item.setPen(pen)
                self.temp_item.setParentItem(self.layer)
            ...
            if self.temp_item is None:
                ...
                pen = QtGui.QPen(QtGui.QColor("#9ece6a"))
                pen.setCosmetic(True)
                self.temp_item.setPen(pen)
                self.temp_item.setParentItem(self.layer)
            path = QtGui.QPainterPath(self.points[0])
