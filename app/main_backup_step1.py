...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        m_underlay.addAction("Scale by ReferenceΓÇª", self.start_underlay_scale_ref)
        m_underlay.addAction("Scale by FactorΓÇª", self.underlay_scale_factor)
        m_underlay.addAction("Scale by DragΓÇª", self.start_underlay_scale_drag)
        ...
        m_modify.addAction("Offset SelectedΓÇª", self.offset_selected_dialog)
        ...
        m_modify.addAction("Fillet (Radius)ΓÇª", self.start_fillet_radius)
        ...
        m_modify.addAction("ChamferΓÇª", self.start_chamfer)
        ...
        m_view.addSeparator()
        act_scale = QtGui.QAction("Set Pixels per FootΓÇª", self); act_scale.triggered.connect(self.set_px_per_ft); m_view.addAction(act_scale)
        act_gridstyle = QtGui.QAction("Grid StyleΓÇª", self); act_gridstyle.triggered.connect(self.grid_style_dialog); m_view.addAction(act_gridstyle)
        ...
        self.cmd = QLineEdit(); self.cmd.setPlaceholderText("Type command (e.g., L, RECT, MOVE)ΓÇª")
    ...
    def _build_left_panel(self):
        ...
        search_group = QtWidgets.QGroupBox("Device Search")
        search_group.setStyleSheet("QGroupBox { font-weight: bold; margin-top: 15px; border: 1px solid #555; border-radius: 4px; padding-top: 15px; background-color: #333336; } QGroupBox::title { padding: 0px 8px; color: #cccccc; font-size: 11pt; }")
        ...
        filter_group = QtWidgets.QGroupBox("Device Filters")
        filter_group.setStyleSheet("QGroupBox { font-weight: bold; margin-top: 20px; border: 1px solid #555; border-radius: 4px; padding-top: 15px; background-color: #333336; } QGroupBox::title { padding: 0px 8px; color: #cccccc; font-size: 11pt; }")
        ...
        # Connect signals
        # Remove direct connection to _filter_device_tree
        self.cmb_category.currentIndexChanged.connect(self._filter_device_tree)
        self.cmb_mfr.currentIndexChanged.connect(self._filter_device_tree)
        self.cmb_type.currentIndexChanged.connect(self._filter_device_tree)
        ...
        # Add FACP placement button with better styling
    ...
    def _populate_filters(self):
        """Populate filter dropdowns with unique values and better organization."""
        # Get unique categories, manufacturers, and types
        categories = set()
        manufacturers = set()
        types = set()
        ...
        for d in self.devices_all:
            ...
            manufacturer = d.get("manufacturer", "(Any)") or "(Any)"
        ...
        # Populate comboboxes with sorted values and clearer labels
        self.cmb_category.clear()
        self.cmb_category.addItems(["All Categories"] + sorted(categories))
        
        self.cmb_mfr.clear()
        self.cmb_mfr.addItems(["All Manufacturers"] + sorted(manufacturers))
        
        self.cmb_type.clear()
        self.cmb_type.addItems(["All Device Types"] + sorted(types))
    ...
    def _filter_device_tree(self):
        ...
        for i in range(self.device_tree.topLevelItemCount()):
            category_item = self.device_tree.topLevelItem(i)
            category_matches = (selected_category == "All Categories" or selected_category == category_item.text(0))
            
            category_visible = False
            for j in range(category_item.childCount()):
                type_item = category_item.child(j)
                type_matches = (selected_type == "All Device Types" or selected_type == type_item.text(0))
                
                type_visible = False
                for k in range(type_item.childCount()):
                    device_item = type_item.child(k)
                    device = device_item.data(0, Qt.UserRole)
                    
                    # Check search text
                    search_matches = not search_text or (
                        search_text in device["name"].lower() or
                        search_text in device["symbol"].lower() or
                        (device.get("part_number") and search_text in device["part_number"].lower())
                    )
                    
                    # Check manufacturer
                    mfr_matches = (selected_mfr == "All Manufacturers" or 
                                  selected_mfr == device.get("manufacturer", "(Any)"))
                    
                    # Check type (already filtered by parent)
                    type_matches_device = (selected_type == "All Device Types" or selected_type == type_item.text(0))
                    
                    # Show/hide device item
                    device_visible = search_matches and mfr_matches and type_matches_device
                    device_item.setHidden(not device_visible)
                    
                    if device_visible:
                        type_visible = True
                
                # Show/hide type item based on whether it has visible children
                type_has_visible_children = type_visible
                type_item.setHidden(not (type_has_visible_children and type_matches))
                
                if type_has_visible_children:
                    category_visible = True
            
            # Show/hide category item based on whether it has visible children
            category_has_visible_children = category_visible
            category_item.setHidden(not (category_has_visible_children and category_matches))
        
        # Expand all items to ensure visibility of filtered results
        self.device_tree.expandAll()
    ...
    def place_facp_panel(self):
        ...
        try:
            ...
            if dialog.exec() == QtWidgets.QDialog.Accepted:
                # Get the configured panel
                panel = dialog.get_panel_configuration()
                
                # Create a device item for the FACP panel
                # For now, we'll use a generic symbol; in the future this could be a custom symbol
                symbol = "FACP"
                ...
                device_item = DeviceItem(x, y, symbol, name, manufacturer, part_number)
