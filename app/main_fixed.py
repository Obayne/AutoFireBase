...
# This file contains long embedded style strings and template content; silence
# E501 line-length warnings to reduce noisy linter failures for intentional long lines.
# ruff: noqa: E501
# noqa: E501
...
# Ensure logging is configured early for consistent output during runs
setup_logging()
...
class CanvasView(QGraphicsView):
    ...
    def _compute_osnap(self, p: QPointF) -> QtCore.QPointF | None:
        ...
        try:
            ...
            # tracking variables removed (not used in current search)
            ...
            for it in items:
                ...
                if isinstance(it, QtWidgets.QGraphicsLineItem):
                    ln = it.line()
                    if self.osnap_end:
                        pts += [QtCore.QPointF(ln.x1(), ln.y1()), QtCore.QPointF(ln.x2(), ln.y2())]
                    if self.osnap_mid:
                        # midpoint
                        pts += [
                            QtCore.QPointF((ln.x1() + ln.x2()) / 2.0, (ln.y1() + ln.y2()) / 2.0)
                        ]
...
class MainWindow(QMainWindow):
    ...
    def load_state(self, data):
        ...
        if "connections" in data:
            self.connections_tree.load_connections(data["connections"], device_map)
        # restore wires
    ...
    # ---------- edit operations ----------
    # Duplicate undo/redo definitions removed (kept earlier definitions)
    ...
    # Duplicate add_viewport removed (kept earlier definition)
    ...
    # ---------- help tools ----------
    # Duplicate show_user_guide removed (kept earlier definition)
...
"""
    ...
    # Duplicate show_about removed (kept earlier definition)
