...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        paper_scene = QtWidgets.QGraphicsScene(0,0,15000,10000) # Separate scene for paperspace
        
        # Add a default viewport to the paperspace scene
        default_viewport = ViewportItem(self.scene, QtCore.QRectF(0,0,1000,800), self)
        paper_scene.addItem(default_viewport)
        ...
        # Apply minimal Paperspace mode visibility tweaks
        if self._is_paperspace_minimal():
            # Hide space controls in minimal mode; keep badge as MODEL SPACE
            try:
                self.space_combo.hide()
                self.space_lock.hide()
                # Add a small status badge to indicate Minimal mode
                try:
                    self.minimal_badge = QtWidgets.QLabel("MINIMAL")
                    self.minimal_badge.setStyleSheet("QLabel { color: #ffaa00; font-weight: bold; }")
                    self.minimal_badge.setToolTip("Paperspace minimal mode active: Paperspace features are disabled.")
                    self.statusBar().addPermanentWidget(self.minimal_badge)
                    # Also set a helpful tooltip on the space badge
                    self.space_badge.setToolTip("Minimal mode: Paperspace is disabled (Model space only)")
                except Exception:
                    pass
            except Exception:
                pass
            # Disable or hide layout actions related to Paperspace features
            try:
                for _act in list(m_layout.actions()):
                    txt = (_act.text() or "").lower()
                    if any(k in txt for k in (
                        ...
                        try:
                            ...
                            pass
                try:
                    scale_menu.setEnabled(False)
                    scale_menu.menuAction().setVisible(False)
                except Exception:
                    pass
            except Exception:
                pass
        ...
        # _build_layers_and_props_dock functionality is integrated into _build_left_panel
        # DXF Layers dock
        self._dxf_layers = {}
    ...
    def _init_sheet_manager(self):
        # Ensure the central tab widget exists
        ...
        self.sheets = []
        # In minimal mode, skip adding paperspace tabs
        if not self._is_paperspace_minimal():
    ...
    def toggle_paper_space(self, on: bool):
        if self._is_paperspace_minimal():
            ...
            try:
                ...
                self.statusBar().showMessage("Paperspace is disabled in minimal mode")
            except Exception:
                pass
            return
    ...
    def _is_paperspace_minimal(self) -> bool:
        try:
            app = QtWidgets.QApplication.instance()
            mode = (app.property("AF_PAPERSPACE_MODE") if app else None) or os.getenv("AF_PAPERSPACE_MODE", "minimal")
