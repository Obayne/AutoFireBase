...
class CanvasView(QGraphicsView):
    ...
    def _update_crosshair(self, sp: QPointF):
        ...
        self.win.statusBar().showMessage(f"x={units.fmt_ft_inches(dx_ft)}   y={units.fmt_ft_inches(dy_ft)}   scale={self.win.px_per_ft:.2f} px/ft  snap={self.win.snap_label}")
    ...
    def mousePressEvent(self, e: QtGui.QMouseEvent):
        ...
        if e.button()==Qt.LeftButton:
            ...
            if getattr(win, "array_tool", None) and win.array_tool.pending:
                if win.array_tool.on_click(sp): e.accept(); return
            # drawing tool
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        pal.setColor(QtGui.QPalette.Text,   QtCore.Qt.white); pal.setColor(QtGui.QPalette.WindowText, QtCore.Qt.white)
        pal.setColor(QtGui.QPalette.Button, QtGui.QColor(48,48,52)); pal.setColor(QtGui.QPalette.ButtonText, QtCore.Qt.white)
        self.setPalette(pal)
        ...
        # toolbar — minimal (kept draw tools in menu as requested)
