...
from app.layout import PAGE_SIZES, PageFrame, TitleBlock, ViewportItem
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        m_help.addSeparator()
        m_help.addAction("Commands…", self._show_command_list)
        ...
        # Quick access help button for commands
        try:
            btn_cmd_help = QPushButton("?")
            btn_cmd_help.setToolTip("Show command list")
            btn_cmd_help.setFixedWidth(24)
            btn_cmd_help.clicked.connect(self._show_command_list)
            cmd_l.addWidget(btn_cmd_help)
        except Exception:
            pass
        sb.addPermanentWidget(cmd_wrap, 1)
        # Enhance visibility of the command bar
        try:
            self.cmd.setClearButtonEnabled(True)
            self.cmd.setMinimumWidth(260)
            ...
            pass
        # Place grid controls to the right of the command bar
    ...
    # ---------- Commands ----------
    def _build_command_map(self, set_draw):
        return {
    ...
    def _show_command_list(self):
        try:
            ...
            return
        dlg = _QtW.QDialog(self)
        dlg.setWindowTitle("Command List")
        lay = _QtW.QVBoxLayout(dlg)
        info = _QtW.QLabel("Type any of these in the Cmd box and press Enter. Aliases are separated by commas.")
        info.setWordWrap(True)
        lay.addWidget(info)
        table = _QtW.QTableWidget()
        table.setColumnCount(2)
        table.setHorizontalHeaderLabels(["Command(s)", "Action"])
        table.horizontalHeader().setStretchLastSection(True)
        # Build grouped list of commands → action label
        groups = [
            ("Line", ["l", "line"]),
        ...
        for i, (label, cmds) in enumerate(groups):
            cmd_item = _QtW.QTableWidgetItem(", ".join(cmds))
            act_item = _QtW.QTableWidgetItem(label)
            table.setItem(i, 0, cmd_item)
            table.setItem(i, 1, act_item)
        table.resizeColumnsToContents()
        lay.addWidget(table)
        bb = _QtW.QDialogButtonBox(_QtW.QDialogButtonBox.Close)
    ...
    def _build_left_panel(self):
        ...
        self.search.setPlaceholderText("Search name / part numberâ€¦")
        self.cmb_mfr = QComboBox()
        self.cmb_type = QComboBox()
        ll_top = QHBoxLayout()
        ll_top.addWidget(QLabel("Manufacturer:"))
        ll_top.addWidget(self.cmb_mfr)
        ll_typ = QHBoxLayout()
        ll_typ.addWidget(QLabel("Type:"))
        ll_typ.addWidget(self.cmb_type)
        ...
        QtGui.QShortcut(
            ...
            self,
            activated=lambda: self.toggle_crosshair(not self.view.show_crosshair),
        )
    ...
    def _build_dxf_layers_dock(self):
        ...
        # Controls row
        row1 = QHBoxLayout()
        self.btn_dxf_color = QPushButton("Set Colorâ€¦")
    ...
    def _run_command(self):
        ...
        m = self._build_command_map(set_draw)
        try:
    ...
    def import_pdf_underlay(self):
        ...
        if not p:
            return
        try:
            from PySide6 import QtPdf  # type: ignore
        except Exception:
        ...
        try:
            ...
            dpi = 96.0
            img = QtGui.QImage(
                int(sz.width() / 72.0 * dpi),
                int(sz.height() / 72.0 * dpi),
    ...
    def underlay_scale_factor(self):
        val, ok = QtWidgets.QInputDialog.getDouble(
            self, "Underlay Scale", "Factor", 1.0, 0.001, 1000.0, 4
        )
        if not ok:
            return
        try:
    ...
    def center_underlay_in_view(self):
        try:
            ...
            self.scene.setSceneRect(
                self.scene.sceneRect().united(ub.adjusted(-200, -200, 200, 200))
            )
            self.push_history()
    ...
    def offset_selected_dialog(self):
        ...
        dlg.setWindowTitle("Offset Selected")
        ...
        spin.setValue(1.0)
        side = QComboBox()
        side.addItems(["Right", "Left"])  # relative to first segment direction
        dup = QCheckBox("Create copy (do not modify original)")
        dup.setChecked(True)
        units = QLabel("feet")
        wrap = QtWidgets.QHBoxLayout()
        wrap.addWidget(spin)
        wrap.addWidget(units)
        field = QWidget()
        field.setLayout(wrap)
        ...
        if dlg.exec() != QtWidgets.QDialog.Accepted:
            return
        dist_ft = float(spin.value())
        right = side.currentText() == "Right"
        make_copy = bool(dup.isChecked())
    ...
    def export_png(self):
        ...
        if self.page_frame and self.page_frame.scene():
            ...
            if (self.prefs.get("page_orient", "Landscape")).lower().startswith("land"):
                w_in, h_in = h_in, w_in
            img = QtGui.QImage(
                int(w_in * dpi), int(h_in * dpi), QtGui.QImage.Format_ARGB32_Premultiplied
            )
            img.fill(QtGui.QColor(255, 255, 255))
            painter = QtGui.QPainter(img)
    ...
    def export_pdf(self):
        ...
        writer.setPageOrientation(
            ...
            if orient.lower().startswith("land")
            else QtGui.QPageLayout.Portrait
        )
        ...
        if self.page_frame and self.page_frame.scene():
            ...
            if rect.isNull():
                rect = QtCore.QRectF(0, 0, 1000, 800)
            page_rect = writer.pageLayout().paintRectPixels(dpi)
            sx = page_rect.width() / rect.width()
            sy = page_rect.height() / rect.height()
            s = min(sx, sy)
            tx = (page_rect.width() - rect.width() * s) / 2
            ty = (page_rect.height() - rect.height() * s) / 2
            painter.translate(tx, ty)
    ...
    def add_page_frame(self):
        dlg = QtWidgets.QDialog(self)
        dlg.setWindowTitle("Add Page Frame")
        form = QtWidgets.QFormLayout(dlg)
        cmb = QComboBox()
        cmb.addItems(list(PAGE_SIZES.keys()))
        cmb.setCurrentText(self.prefs.get("page_size", "Letter"))
        ori = QComboBox()
        ori.addItems(["Portrait", "Landscape"])
        ori.setCurrentText(self.prefs.get("page_orient", "Landscape"))
        spm = QDoubleSpinBox()
        spm.setRange(0.0, 2.0)
        spm.setSingleStep(0.1)
        spm.setValue(float(self.prefs.get("page_margin_in", 0.5)))
        form.addRow("Size:", cmb)
        form.addRow("Orientation:", ori)
        form.addRow("Margin (in):", spm)
        bb = QtWidgets.QDialogButtonBox(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        form.addRow(bb)
        ...
        if dlg.exec() != QtWidgets.QDialog.Accepted:
            return
        self.prefs["page_size"] = cmb.currentText()
        self.prefs["page_orient"] = ori.currentText()
        self.prefs["page_margin_in"] = float(spm.value())
        save_prefs(self.prefs)
        if self.page_frame and self.page_frame.scene():
            try:
                self.scene.removeItem(self.page_frame)
            except Exception:
                pass
            self.page_frame = None
        pf = PageFrame(
            self.px_per_ft,
            size_name=self.prefs["page_size"],
            orientation=self.prefs["page_orient"],
            margin_in=self.prefs["page_margin_in"],
        )
    ...
    def add_or_update_title_block(self):
        ...
        dlg = QtWidgets.QDialog(self)
        dlg.setWindowTitle("Title Block")
        form = QtWidgets.QFormLayout(dlg)
        ...
        if dlg.exec() != QtWidgets.QDialog.Accepted:
            return
        meta = {
            "project": ed_project.text(),
            "address": ed_address.text(),
            "sheet": ed_sheet.text(),
            "date": ed_date.text(),
            "by": ed_by.text(),
        }
        self.prefs.update({"proj_" + k: v for k, v in meta.items()})
        save_prefs(self.prefs)
        # Add or update
    ...
    def page_setup_dialog(self):
        ...
        form = QtWidgets.QFormLayout(dlg)
        size = QComboBox()
        size.addItems(list(PAGE_SIZES.keys()))
        size.setCurrentText(self.prefs.get("page_size", "Letter"))
        orient = QComboBox()
        orient.addItems(["Portrait", "Landscape"])
        orient.setCurrentText(self.prefs.get("page_orient", "Landscape"))
        margin = QDoubleSpinBox()
        margin.setRange(0.0, 2.0)
        margin.setDecimals(2)
        margin.setSingleStep(0.1)
        margin.setValue(float(self.prefs.get("page_margin_in", 0.5)))
        form.addRow("Size:", size)
        form.addRow("Orientation:", orient)
        form.addRow("Margin (in):", margin)
        bb = QtWidgets.QDialogButtonBox(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        form.addRow(bb)
        ...
        if self.title_block and self.title_block.scene():
            ...
            meta = {
                ...
                "sheet": self.prefs.get("proj_sheet", ""),
    ...
    def _on_space_combo_changed(self, idx: int):
        if getattr(self, "space_lock", None) is not None and self.space_lock.isChecked():
            # Revert change if locked
            try:
                self.space_combo.blockSignals(True)
                self.space_combo.setCurrentIndex(1 if getattr(self, "in_paper_space", False) else 0)
            finally:
    ...
    def _refresh_sheets_list(self):
        ...
        if self.paper_scene:
            try:
                idx = next(
                    (i for i, s in enumerate(self.sheets) if s.get("scene") is self.paper_scene), 0
                )
    ...
    def export_sheets_pdf(self):
        ...
        try:
            ...
            for sheet in self.sheets:
                ...
                size_in = PAGE_SIZES.get(
                    self.prefs.get("page_size", "Letter"), PAGE_SIZES["Letter"]
                )
                orient = self.prefs.get("page_orient", "Landscape")
    ...
    def set_print_scale(self, inches_per_ft: float):
        self.prefs["print_in_per_ft"] = float(inches_per_ft)
        save_prefs(self.prefs)
    ...
    def set_print_scale_custom(self):
        val, ok = QtWidgets.QInputDialog.getDouble(
            ...
            "Custom Print Scale",
            "Inches per foot",
            float(self.prefs.get("print_in_per_ft", 0.125)),
            0.01,
            12.0,
            3,
        )
