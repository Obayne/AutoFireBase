...
def scaffold_for_issue(title: str, labels: list[str]):
    ...
    if any(lab.startswith("area:cad_core") for lab in labels):
    ...
    if any(lab.startswith("area:backend") for lab in labels):
    ...
    if any(lab.startswith("area:frontend") for lab in labels):
        if "input" in lowered:
