...
# Test the exact query from fetch_panels
...
try:
    cur.execute(
        """
        SELECT p.id, p.manufacturer_id, p.model, p.name, p.panel_type, p.max_devices, p.properties_json, m.name as manufacturer_name
        FROM panels p
